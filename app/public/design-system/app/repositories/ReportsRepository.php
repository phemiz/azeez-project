    ...
    public function getReportData(string $type, array $filters, string $sortBy = 'created_at', string $sortOrder = 'DESC', int $limit = 10, int $offset = 0): array {
        ...
        switch ($type) {
            ...
            case 'security':
                ...
                $fields = "id, severity, status, triggered_rule, created_at";
                if (!empty($filters['search'])) {
                    $where[] = "(triggered_rule LIKE ? OR severity LIKE ?)";
                    $params[] = "%{$filters['search']}%";
                    $params[] = "%{$filters['search']}%";
                }
                break;
            ...
            case 'encryptions':
                ...
                $fields = "id, recipient, algorithm, risk_grade, created_at";
                if (!empty($filters['search'])) {
                    $where[] = "(recipient LIKE ? OR algorithm LIKE ?)";
                    $params[] = "%{$filters['search']}%";
                    $params[] = "%{$filters['search']}%";
                }
                break;
