...
<div class="flex items-center justify-center min-h-[75vh]">
    <div class="w-full max-w-lg p-8 rounded-2xl cyber-card border border-cyan-500/20 relative overflow-hidden">
        ...
        <div class="text-center mb-6 relative z-10">
            <h1 class="text-xl font-bold tracking-tight text-white mb-2 font-mono uppercase">Register New Account</h1>
            <p class="text-xs" style="color: var(--color-foreground-muted);">Create your new operator profile</p>
        </div>
        ...
        <form id="registerForm" class="space-y-4 relative z-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <!-- Phone (GSM Number) -->
                <div>
                    <label for="phone" class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Phone Number</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center" style="color: var(--color-foreground-muted);">
            ...
            <!-- Email -->
            <div>
                <label for="email" class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Email Address</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center" style="color: var(--color-foreground-muted);">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <!-- Confirm Password -->
                <div>
                    <label for="confirm_password" class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Confirm Password</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center" style="color: var(--color-foreground-muted);">
                            <i data-lucide="lock" class="w-4 h-4"></i>
                        </span>
            ...
            <div>
                ...
                <div class="flex justify-between items-center text-[10px] mt-1 font-mono">
                    <span id="strengthText" style="color: var(--color-foreground-muted);">Password Strength</span>
            ...
            <div class="flex items-start space-x-2.5 pt-2">
                ...
                <label for="terms" class="text-2xs select-none cursor-pointer" style="color: var(--color-foreground-muted);">
                    I agree to the system security and logging policies.
                </label>
            </div>
        ...
        <div class="text-center mt-6 relative z-10 font-mono">
            ...
            <a href="<?= APP_URL ?>/login" class="text-xs hover:underline font-bold transition-colors" style="color: var(--color-primary);">Return to Login Page</a>
        </div>
    </div>
</div>
...
document.getElementById('password').addEventListener('input', function() {
    ...
    if (val.length === 0) {
        ...
        txt.textContent = 'Strong Crypto Passcode';
        txt.style.color = '#10b981';
    }
});

// Ajax registration post submit
document.getElementById('registerForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok && data.status === 'success') {
            ...
            submitBtn.innerHTML = `<span>Create Account</span><i data-lucide="shield-check" class="w-4 h-4"></i>`;
            lucide.createIcons();
        }
    } catch (err) {
        alertBox.className = "p-4 rounded-xl text-xs font-mono font-medium border border-rose-500/30 bg-rose-500/10 text-rose-400 flex items-start space-x-2";
