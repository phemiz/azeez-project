...
class AuthController extends Controller {
    ...
    public function login(): void {
        ...
        try {
            ...
            Session::set('login_method', 'credentials');
    ...
    public function verifyOTP(): void {
        $user = Session::get('user');
        if (!$user) {
            $this->json(['status' => 'error', 'message' => 'Your login session expired. Please sign in again.'], 401);
        }

        $code = $this->getPost('otp_code');
        if (empty($code)) {
            $this->json(['status' => 'error', 'message' => 'Login code is required.'], 400);
        }
        ...
        if ($this->otpService->verifyOTP($user['id'], $code)) {
            ...
            $this->json(['status' => 'error', 'message' => 'The code is incorrect or has expired.'], 400);
        }
    }
    ...
    public function resendOTP(): void {
        ...
        try {
            ...
            $this->json(['status' => 'error', 'message' => 'Could not create a new login code.'], 500);
        }
    }
