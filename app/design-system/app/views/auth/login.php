...
<div class="flex items-center justify-center min-h-[70vh]">
    <div class="w-full max-w-md p-8 rounded-2xl cyber-panel border border-cyan-500/20 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            <h1 class="text-2xl font-bold tracking-tight text-white mb-2">Secure Terminal Access</h1>
            <p class="text-sm text-gray-400">Authorization required to decrypt GSM payloads</p>
        </div>
        ...
        <form id="loginForm" class="space-y-6 relative z-10">
            ...
            <!-- Username -->
            <div>
                <label for="username" class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Terminal Username</label>
                <div class="relative">
            ...
            <!-- Password -->
            <div>
                <label for="password" class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Passcode</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center" style="color: var(--color-foreground-muted);">
            ...
            <div class="flex items-center justify-between mt-2">
                <div class="flex items-center space-x-2">
                    ...
                    <label for="remember" class="text-xs select-none cursor-pointer" style="color: var(--color-foreground-muted);">Remember Node</label>
                </div>
            </div>

            <!-- Alert Container -->
            <div id="alertBox" class="hidden p-4 rounded-xl text-xs font-medium border flex items-start space-x-2 animate-bounce">
        ...
        <div class="text-center mt-6 relative z-10">
            <span class="text-xs text-gray-500">New base operator? </span>
            <a href="<?= APP_URL ?>/register" class="text-xs text-cyan-400 hover:text-cyan-300 font-semibold transition-colors">Request Account Registration</a>
        </div>
    </div>
</div>
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok && data.status === 'success') {
            ...
            submitBtn.innerHTML = `<span>Verify Credentials</span><i data-lucide="arrow-right" class="w-4 h-4"></i>`;
            lucide.createIcons();
        }
    } catch (err) {
        alertBox.className = "p-4 rounded-xl text-xs font-medium border border-rose-500/30 bg-rose-500/10 text-rose-400 flex items-start space-x-2";
