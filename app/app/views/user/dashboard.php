...
<div class="space-y-8">
        ...
        <div class="cyber-card col-span-2 flex flex-col justify-between relative overflow-hidden">
            ...
            <div>
                ...
                <p class="text-xs mt-1" style="color: var(--color-foreground-muted);">
                    Status: Node Active &middot; System Security Heuristics Engaged
                </p>
            </div>
            ...
            <div class="mt-6 pt-4 border-t text-2xs font-mono" style="border-color: var(--color-border); color: var(--color-foreground-muted);">
                <?php if ($lastLogin): ?>
                    LAST ACCESS PATH: <span class="text-white"><?= htmlspecialchars($lastLogin['ip_address']) ?></span> ON <span class="text-white"><?= date('Y-m-d H:i:s', strtotime($lastLogin['created_at'])) ?></span>
                <?php else: ?>
                    NODE STATE: First session initialized. Security parameters established.
                <?php endif; ?>
            </div>
        </div>

        <!-- Active Session Risk rating gauge -->
        <div class="cyber-card flex flex-col justify-between" style="background-color: var(--color-surface);">
            <div>
                <span class="text-2xs font-bold uppercase tracking-wider block" style="color: var(--color-foreground-muted);">Session Risk Rating</span>
                <?php 
                ...
                <div class="mt-4 flex items-baseline gap-2">
                    ...
                    <span class="text-xs uppercase font-mono font-bold" style="color: var(--color-foreground-muted);">
                        (<?= $risk >= 70 ? 'CRITICAL' : ($risk >= 30 ? 'SUSPICIOUS' : 'SECURE') ?>)
                    </span>
                </div>
            </div>
            ...
            <div class="mt-4 border-t pt-2" style="border-color: var(--color-border);">
                <button onclick="toggleRiskDetails()" class="text-3xs uppercase font-mono tracking-widest text-cyan-400 hover:underline flex items-center gap-1 cursor-pointer">
                ...
                <div id="riskDetailsBox" class="hidden mt-2 space-y-1 text-[10px] font-mono" style="color: var(--color-foreground-muted);">
                    ...
                    <div class="flex justify-between"><span>UNKNOWN IP:</span><span class="text-white font-bold"><?= $riskAssessment['breakdown']['unknown_ip'] ?>%</span></div>
                    <div class="flex justify-between"><span>RAPID LOGINS:</span><span class="text-white font-bold"><?= $riskAssessment['breakdown']['rapid_logins'] ?>%</span></div>
                    <div class="flex justify-between"><span>GEOGRAPHIC JUMP:</span><span class="text-white font-bold"><?= $riskAssessment['breakdown']['impossible_travel'] ?>%</span></div>
                    <div class="flex justify-between"><span>EXFILTRATION VOL:</span><span class="text-white font-bold"><?= $riskAssessment['breakdown']['encryption_frequency'] ?>%</span></div>
                    <div class="flex justify-between"><span>SESSION ANOMALY:</span><span class="text-white font-bold"><?= $riskAssessment['breakdown']['session_anomaly'] ?>%</span></div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
        ...
        <div class="cyber-card flex items-center justify-between">
            <div>
                <span class="text-2xs font-bold uppercase tracking-wider block" style="color: var(--color-foreground-muted);">GSM Envelopes</span>
        ...
        <div class="cyber-card flex items-center justify-between">
            <div>
                <span class="text-2xs font-bold uppercase tracking-wider block" style="color: var(--color-foreground-muted);">Node Profile</span>
        ...
        <div class="cyber-card flex items-center justify-between">
            <div>
                <span class="text-2xs font-bold uppercase tracking-wider block" style="color: var(--color-foreground-muted);">Peak Alerts</span>
    ...
    <div class="cyber-card p-6">
        <h3 class="text-sm font-bold uppercase tracking-wider text-white mb-4 font-mono">10-Incident Threat Telemetry History</h3>
        <div class="h-64">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
            ...
            <div class="flex p-1.5 rounded-xl border" style="background-color: var(--color-surface); border-color: var(--color-border);">
                <button onclick="switchTab('encrypt')" id="btn-tab-encrypt" 
                ...
                <button onclick="switchTab('decrypt')" id="btn-tab-decrypt" 
            ...
            <div id="panel-encrypt" class="cyber-card space-y-4">
                <h2 class="text-sm font-bold text-white flex items-center gap-2 font-mono uppercase">
                    ...
                    <span>Payload Enveloping (AES-256-CBC)</span>
                </h2>
                ...
                <form id="encryptForm" class="space-y-4">
                    ...
                    <div class="p-4 rounded-xl border space-y-3" style="background-color: rgba(0,0,0,0.15); border-color: var(--color-border);">
                        <span class="text-2xs text-gray-400 block font-mono font-bold tracking-widest uppercase">GSM Satellites Simulation Context</span>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="text-2xs font-semibold block mb-1" style="color: var(--color-primary);">Service Center (SMSC)</label>
                                <input type="text" name="smsc" value="234803000000" placeholder="e.g. +234803..." 
                                       class="cyber-input font-mono text-xs" />
                                <span class="text-[8px]" style="color: var(--color-foreground-muted);">Use '0000000000' to simulate an IMSI Catcher</span>
                            </div>
                            <div>
                                ...
                                <span class="text-[8px]" style="color: var(--color-foreground-muted);">Type 0 Silent SMS triggers tracker warnings</span>
                            </div>
                        </div>
                    </div>

                    <div>
                        <label class="block text-2xs font-bold uppercase tracking-wider mb-1.5" style="color: var(--color-primary);">Message Payload</label>
                        <textarea name="message" rows="3" required placeholder="Type confidential data payload..." class="cyber-input"></textarea>
                    </div>

                    <button type="submit" id="btn-encrypt-submit" class="btn-primary w-full justify-center">
                ...
                <div id="encryptResult" class="hidden p-4 rounded-xl border space-y-3" style="background-color: rgba(0,0,0,0.1); border-color: var(--color-primary);">
                    <span class="text-xs font-mono font-bold flex items-center gap-1.5" style="color: var(--color-primary);">
                        ...
                        <span>Cryptographic Envelope Encapsulated</span>
                    </span>
                    
                    <div class="space-y-2">
                        <label class="text-[10px] block font-mono" style="color: var(--color-foreground-muted);">Ciphertext Payload:</label>
                    ...
                    <div class="grid grid-cols-3 gap-2 text-[10px] font-mono">
                        <div>
                            <span style="color: var(--color-foreground-muted);" class="block">IV:</span>
                    ...
                    <button onclick="copyCredentials()" class="btn-secondary w-full justify-center">
                        <i data-lucide="copy" class="w-3.5 h-3.5"></i>
                        <span>Copy Envelope Parameters</span>
                    </button>
                </div>
            </div>
            ...
            <div id="panel-decrypt" class="cyber-card space-y-4 hidden">
                <h2 class="text-sm font-bold text-white flex items-center gap-2 font-mono uppercase">
                    ...
                    <span>Envelope Decapsulation Terminal</span>
                </h2>
                ...
                <form id="decryptForm" class="space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="ciphertext" rows="2" required placeholder="Paste Envelope Ciphertext" class="cyber-input font-mono text-xs"></textarea>
                    </div>

                    <div class="grid grid-cols-3 gap-2">
                        <div>
                            <label class="text-[10px] font-mono font-semibold" style="color: var(--color-primary);">IV</label>
                            <input type="text" name="iv" required placeholder="Base64 IV" class="cyber-input font-mono text-xs" />
                        </div>
                        <div>
                            <label class="text-[10px] font-mono font-semibold" style="color: var(--color-primary);">Salt</label>
                            <input type="text" name="salt" required placeholder="Base64 Salt" class="cyber-input font-mono text-xs" />
                        </div>
                        <div>
                            <label class="text-[10px] font-mono font-semibold" style="color: var(--color-primary);">Signature</label>
                            <input type="text" name="signature" required placeholder="HMAC Signature" class="cyber-input font-mono text-xs" />
                        </div>
                    </div>
                    ...
                    <button type="submit" id="btn-decrypt-submit" class="btn-primary w-full justify-center">
                        <i data-lucide="unlock" class="w-4 h-4"></i>
        ...
        <div class="space-y-6">
            ...
            <div id="aiWarningCard" class="cyber-card bg-cyan-500/5 space-y-3 relative overflow-hidden">
                <!-- Overlay glow -->
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h3 class="text-xs font-bold text-white uppercase tracking-wider font-mono">AI Threat Intel Recommendations</h3>
                        <span class="text-[10px] block" style="color: var(--color-foreground-muted);">Real-time cellular anomaly detection</span>
                    </div>
                </div>

                <div class="space-y-2" id="aiReportBox">
                    <p class="text-xs leading-relaxed font-mono" style="color: var(--color-primary);">
                        State: Normal. Cellular link telemetry shows no IMSI anomalies or active type 0 silent SMS trackers. Messages encrypted inside this session are safe.
                    </p>
                </div>
            </div>
            ...
            <div class="cyber-card space-y-4">
                <h3 class="text-xs font-bold uppercase tracking-wider text-white flex items-center justify-between font-mono">
                    <span>Recent Security Auditing Logs</span>
                    ...
                    <table class="w-full text-left text-xs border-collapse" style="color: var(--color-foreground);">
                        <thead>
                            <tr class="border-b font-mono text-[10px]" style="border-color: var(--color-border); color: var(--color-primary);">
                                ...
                                <th class="pb-2 font-medium">Threat Classification</th>
                            </tr>
                        </thead>
...
document.getElementById('encryptForm').addEventListener('submit', async function(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = `<span>Encapsulating...</span>`;
    ...
    try {
        ...
        if (response.ok && data.status === 'success') {
            ...
            ToastManager.show("GSM envelope created successfully.");
...
document.getElementById('decryptForm').addEventListener('submit', async function(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = `<span>Decapsulating...</span>`;
    ...
    try {
        ...
        if (response.ok && data.status === 'success') {
            ...
            ToastManager.show("Envelope decrypted successfully.");
        } else {
            ToastManager.show(data.message || 'Decryption failure.', 'error');
        }
    } catch(e) {
        ToastManager.show('Decryption network execution failure.', 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = `<i data-lucide="unlock" class="w-4 h-4"></i><span>Decrypt Envelope</span>`;
        lucide.createIcons();
    }
});
